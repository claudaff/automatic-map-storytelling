...
# Python script to fine-tune CLIP for the century information
...
date_captions = np.load("classicalMapsCaptionsDate.npy")
date_paths = np.load("classicalMapsPathsDate.npy")

# SUBSAMPLING

date_captions_no_19th_century = date_captions[date_captions != "19th century"]
indices_no_19th_century = np.argwhere(date_captions != "19th century")

date_paths_no_19th_century = date_paths[indices_no_19th_century]

print(len(date_captions_no_19th_century))
print(len(date_paths_no_19th_century))

date_captions_19th_century = date_captions[date_captions == "19th century"]
indices_19th_century = np.argwhere(date_captions == "19th century")

date_paths_19th_century = date_paths[indices_19th_century]

random.seed(24)
subsampled_captions_19th_century = sample(
    list(date_captions_19th_century), k=221
)  # Subsample since too many maps from 19th century
random.seed(24)
subsampled_paths_19th_century = sample(list(date_paths_19th_century), k=221)

subsample_captions = np.array(subsampled_captions_19th_century)
subsample_paths = np.array(subsampled_paths_19th_century)

date_captions = np.concatenate((date_captions_no_19th_century, subsample_captions))
paths = np.concatenate((date_paths_no_19th_century, subsample_paths))

date_paths = []
for sublist in paths:
    date_paths.extend(sublist)

print(Counter(date_captions))
...
captions = date_captions
image_paths = date_paths
...
test_dataset = Countries(val_captions, val_image_paths)
...
def train_model(
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        if val_metrics < best_score:
            ...
            if os.path.exists(Path(checkpoint_path) / "CLIPCentury.pt"):
                os.remove(Path(checkpoint_path) / "CLIPCentury.pt")
            best_score = val_metrics
            torch.save(model.state_dict(), Path(checkpoint_path) / "CLIPCentury.pt")
