...
# Python script to fine-tune CLIP for topographic locations
...
area_captions = np.load("classicalMapsCaptionsArea.npy")
area_paths = np.load("classicalMapsPathsArea.npy")
...
def train_model(
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        if val_metrics < best_score:
            ...
            if os.path.exists(Path(checkpoint_path) / "CLIPLocTopo.pt"):
                os.remove(Path(checkpoint_path) / "CLIPLocTopo.pt")
            best_score = val_metrics
            torch.save(model.state_dict(), Path(checkpoint_path) / "CLIPLocTopo.pt")
