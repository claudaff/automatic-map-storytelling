...
# Python script to fine-tune CLIP for map types
...
class_pic_captions = np.load("classPictorialCaptions.npy")
class_pic_paths = np.load("classPictorialPaths.npy")


print(len(class_pic_captions))
print(len(class_pic_paths))

print(Counter(class_pic_captions))

# SUBSAMPLING

captions_non_pic = class_pic_captions[class_pic_captions != "pictorial map"]
indices_non_pic = np.argwhere(class_pic_captions != "pictorial map")

paths_non_pic = class_pic_paths[indices_non_pic]


captions_pic = class_pic_captions[class_pic_captions == "pictorial map"]
indices_pic = np.argwhere(class_pic_captions == "pictorial map")

paths_pic = class_pic_paths[indices_pic]


random.seed(24)
subsampled_captions_pic = sample(
    list(captions_pic), k=1334
)  # Subsample since too many pictorial maps
random.seed(24)
subsampled_paths_pic = sample(list(paths_pic), k=1334)

subsample_captions = np.array(subsampled_captions_pic)
subsample_paths = np.array(subsampled_paths_pic)

class_pic_captions = np.concatenate((captions_non_pic, subsample_captions))
paths = np.concatenate((paths_non_pic, subsample_paths))

class_pic_paths = []
for sublist in paths:
    class_pic_paths.extend(sublist)
...
captions = class_pic_captions
image_paths = class_pic_paths
...
train_dataset = MapType(train_captions_final, train_image_paths_final)
val_dataset = MapType(val_captions, val_image_paths)
