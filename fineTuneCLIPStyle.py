...
# Python script to fine-tune CLIP for style information
...
note_captions = np.load("classicalMapsCaptionsNote.npy")
note_paths = np.load("classicalMapsPathsNote.npy")

# SUBSAMPLING

note_captions_no_handcolor = note_captions[note_captions != "hand colored"]
indices_no_handcolor = np.argwhere(note_captions != "hand colored")

note_paths_no_handcolor = note_paths[indices_no_handcolor]

print(len(note_captions_no_handcolor))
print(len(note_paths_no_handcolor))

note_captions_handcolor = note_captions[note_captions == "hand colored"]
indices_handcolor = np.argwhere(note_captions == "hand colored")

note_paths_handcolor = note_paths[indices_handcolor]

random.seed(24)
subsampled_captions = sample(
    list(note_captions_handcolor), k=224
)  # Subsample since too many maps with color
random.seed(24)
subsampled_paths = sample(list(note_paths_handcolor), k=224)

subsample_captions = np.array(subsampled_captions)
subsample_paths = np.array(subsampled_paths)

note_captions = np.concatenate((note_captions_no_handcolor, subsample_captions))
paths = np.concatenate((note_paths_no_handcolor, subsample_paths))

note_paths = []
for sublist in paths:
    note_paths.extend(sublist)

print(Counter(note_captions))
...
captions = note_captions
image_paths = note_paths
...
def train_model(
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        if val_metrics < best_score:
            ...
            if os.path.exists(Path(checkpoint_path) / "CLIPStyle.pt"):
                os.remove(Path(checkpoint_path) / "CLIPStyle.pt")
            best_score = val_metrics
            torch.save(model.state_dict(), Path(checkpoint_path) / "CLIPStyle.pt")
