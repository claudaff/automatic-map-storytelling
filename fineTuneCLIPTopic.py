...
# Python script to fine-tune CLIP for pictorial topics
...
train_dataset = PictorialMaps(train_captions_final, train_image_paths_final)
val_dataset = PictorialMaps(val_captions, val_image_paths)
...
def train_model(
    ...
    for epoch in tqdm(range(n_epochs)):
        ...
        if val_metrics < best_score:
            ...
            if os.path.exists(Path(checkpoint_path) / "CLIPTopic.pt"):
                os.remove(Path(checkpoint_path) / "CLIPTopic.pt")
            best_score = val_metrics
            torch.save(model.state_dict(), Path(checkpoint_path) / "CLIPTopic.pt")
